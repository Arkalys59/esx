if not Config.esx_society then
		...
		ESX.TriggerServerCallback('esx_society:getJob', function(job)
			...
			ESX.OpenContext("right", elements, function(menu,element)
				local elements = {
					...
					{icon = "fas fa-wallet", title = TranslateCap('amount_title'), input = true, inputType = "number", inputPlaceholder = TranslateCap('change_salary_placeholder'), inputMin = 1, inputMax = ConfigSociety.MaxSalary, name = "gradesalary"},
					{icon = "fas fa-check", title = TranslateCap('confirm'), value = "confirm"}, 
					{icon = "fas fa-arrow-left", title = TranslateCap('return'), value = "return"}
				}
